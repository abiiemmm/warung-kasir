...
/** ownerOnly: menu yang menyentuh data seluruh warung, disembunyikan dari kasir. */
const links = [
  ...
  { href: "/logs", label: "Logs", icon: "History", ownerOnly: true },
      ...
      >
        ...
        <div className="mt-auto px-3 pb-4">
          <div className="rounded-lg border border-white/[0.06] bg-white/[0.03] px-3 py-3">
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center text-[13px] font-semibold text-white/80 shrink-0">
                {user?.name.charAt(0).toUpperCase()}
              </div>
              <div className="min-w-0 flex-1">
                <p className="text-[13px] font-medium text-white/90 truncate">{user?.name}</p>
                <p className="text-[11px] text-white/40 font-medium">{isOwner ? "Pemilik" : "Kasir"}</p>
              </div>
              <button
                onClick={() => logout()}
                title="Keluar"
