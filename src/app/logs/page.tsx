...
export default function LogsPage() {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-6xl mx-auto">
      <div className="mb-8">
        <h1 className="text-[22px] font-semibold text-[#1c1917] tracking-tight">Log Aktivitas</h1>
        <p className="text-sm text-[#78716c] mt-1">Riwayat semua perubahan yang terjadi di sistem</p>
      </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[#f5f5f4]">
                  <th className="text-left px-3 sm:px-6 py-3.5 text-xs font-semibold text-[#78716c] uppercase tracking-wider">Waktu</th>
                  <th className="text-left px-3 sm:px-6 py-3.5 text-xs font-semibold text-[#78716c] uppercase tracking-wider">Aksi</th>
                  <th className="text-left px-3 sm:px-6 py-3.5 text-xs font-semibold text-[#78716c] uppercase tracking-wider">Entitas</th>
                  <th className="text-left px-3 sm:px-6 py-3.5 text-xs font-semibold text-[#78716c] uppercase tracking-wider">Detail</th>
                </tr>
              </thead>
              <tbody>
                {data.map((log) => (
                  <tr key={log.id} onClick={() => setSelectedLog(log)} className="border-b border-[#f5f5f4] hover:bg-[#fafaf9] transition-colors cursor-pointer">
                    <td className="px-3 sm:px-6 py-4 whitespace-nowrap">
                      <div className="flex flex-col">
                        <span className="text-[#44403c] font-medium text-xs">
                    ...
                    <td className="px-3 sm:px-6 py-4">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-medium border ${actionColors[log.action] || "bg-[#f5f5f4] text-[#78716c] border-[#e7e5e4]"}`}>
                        {actionLabels[log.action] || log.action}
                      </span>
                    </td>
                    <td className="px-3 sm:px-6 py-4 whitespace-nowrap">
                      <div className="flex flex-col">
                        <span className="text-[#44403c] font-medium text-xs">{entityLabels[log.entity] || log.entity}</span>
                        <span className="text-[#a8a29e] text-[11px] truncate max-w-[120px] sm:max-w-[140px]">{log.entityName}</span>
                      </div>
                    </td>
                    <td className="px-3 sm:px-6 py-4 text-[#78716c] text-xs">{log.details}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="bg-white rounded-2xl shadow-xl border border-[#e7e5e4] w-full max-w-lg mx-4" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between p-5 border-b border-[#e7e5e4]">
              <div className="flex items-center gap-3">
                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-medium border ${actionColors[selectedLog.action] || "bg-[#f5f5f4] text-[#78716c] border-[#e7e5e4]"}`}>
                  {actionLabels[selectedLog.action] || selectedLog.action}
                </span>
                <span className="inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-medium border bg-[#f5f5f4] text-[#78716c] border-[#e7e5e4]">
                  {entityLabels[selectedLog.entity] || selectedLog.entity}
                </span>
              </div>
              <button onClick={() => setSelectedLog(null)} className="p-1.5 rounded-lg hover:bg-[#f5f5f4] transition-colors">
                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg>
              </button>
            </div>
            <div className="p-5 space-y-4">
              <div>
                <label className="block text-xs font-medium text-[#78716c] mb-1.5">Waktu</label>
              ...
              <div>
                <label className="block text-xs font-medium text-[#78716c] mb-1.5">Nama Entitas</label>
                <p className="text-sm text-[#1c1917] font-medium">{selectedLog.entityName}</p>
              </div>
              <div>
                <label className="block text-xs font-medium text-[#78716c] mb-1.5">Detail</label>
                <p className="text-sm text-[#1c1917] leading-relaxed">{selectedLog.details}</p>
              </div>
              <div>
                <label className="block text-xs font-medium text-[#78716c] mb-1.5">ID Log</label>
                <p className="text-xs text-[#a8a29e] font-mono">{selectedLog.id}</p>
              </div>
            </div>
          </div>
        </div>
